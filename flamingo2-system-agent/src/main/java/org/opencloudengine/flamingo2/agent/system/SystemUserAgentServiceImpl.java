...
public class SystemUserAgentServiceImpl implements SystemUserAgentService {
     ...
     */
    private Logger logger = LoggerFactory.getLogger(SystemUserAgentServiceImpl.class);
    ...
    @Override
    public boolean createUser(String username, String name, String password, String linuxUserHome) {
        try {
            // cli : useradd -b {} -m -s /bin/bash -c {} -p $(echo {} | openssl passwd -1 -stdin) {}
            String userCli = MessageFormatter.arrayFormat(createCli, new String[]{linuxUserHome, name, password, username}).getMessage();

            logger.debug("시스템 사용자를 생성합니다. CLI : {}", userCli);
            ...
            expect.sendLine(userCli).close();

            logger.debug("시스템 사용자가 생성되었습니다. {}", username);
