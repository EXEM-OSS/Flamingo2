 ...
 */
Ext.define('Flamingo2.view.tajo.metastore.MetastoreController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.tajoMetastoreController',

    requires: [
        'Flamingo2.view.fs.hdfs.simple.SimpleHdfsBrowser',
        'Flamingo2.view.tajo.metastore._CreateDatabase'
    ],

    listen: {
        controller: {
            'tajoMetastoreController': {
            ...
            'tajoMetastoreCreateTableController': {
                refreshTable: 'onTableRefreshClick'
            },
            'tajoCreateDatabaseController': {
    ...
    onAfterrender: function (view) {
        ...
        this.getViewModel().getStore('databases').load({
            ...
            callback: function (records, operation, success) {
                if (!success) {
                    error(message.msg('tajo.error'), message.msg('common.msg.get_databases'));
                }
            }
        });
    },
    ...
    onCreateDatabaseClick: function () {
        Ext.create('Flamingo2.view.tajo.metastore._CreateDatabase').show();
    },
    ...
    onDropDatabaseClick: function () {
        ...
        Ext.MessageBox.show({
            ...
            message: message.msg('tajo.msg.db_drop'),
    ...
    onTableRefreshClick: function () {
        var refs = query('tajoMetastoreViewport').getReferences();
        var database = refs.cbxDatabase.getValue();
    ...
    onTableCreateClick: function () {
        ...
        Ext.create('Flamingo2.view.tajo.metastore._CreateTable', {
    ...
    onTableDropClick: function () {
        ...
        try {
            record = refs.grdTable.getSelectionModel().getSelected().startCell.record;
        } catch (err) {
            record = refs.grdTable.getSelectionModel().getSelection()[0];
        }
        var params = {
            clusterName: ENGINE.id,
            database: refs.cbxDatabase.getValue(),
