...
/**
 * Custom Mapreduce를 실행하는 태스크
 *
 * @author Haneul, Kim
 * @since 2.1.0
 */
public class CustomMapreduceTask extends InterceptorAbstractTask {

    /**
     * SLF4J Logging
     */
    private Logger logger = LoggerFactory.getLogger(CustomMapreduceTask.class);
    ...
    private void injectLibJars(List<String> command) {
        if (variable.get("path") != null && !isEmpty(variable.get("path").toString())) {
            ...
            command.add(StringUtils.join(dependencies, ","));
        }
    }
    ...
    private void injectKeyValues(List<String> command) {
        ...
        if (variable.get("hadoopKeys") != null &&
                variable.get("hadoopValues") != null &&
                !isEmpty(String.valueOf(variable.get("hadoopKeys"))) &&
                !isEmpty(String.valueOf(variable.get("hadoopValues")))) {
        ...
        command.add(StringUtils.join(keyvalues, " "));
    }
